...
public struct CircularBuffer<Element>: CustomStringConvertible {
    public enum EnqueueError: Swift.Error {
        case droppedNewest(Element)
        case droppedOldest(Element)
    }
...
extension CircularBuffer {
    ...
    /// Amortized *O(1)*
    @inlinable
    public mutating func append(_ value: Element) throws {
        if count == capacity {
            throw EnqueueError.droppedNewest(value)
        }
...
//        }
    ...
    /// Amortized *O(1)*
    @inlinable
    public mutating func prepend(_ value: Element) throws {
        if count == capacity {
            throw EnqueueError.droppedNewest(value)
        }
