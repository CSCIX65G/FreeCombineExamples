...
class CircularBufferTests: XCTestCase {
    func testTrivial() throws {
        ...
        do { try ring.append(1) }
        catch { XCTFail("Should have been able to append") }
        XCTAssertTrue(ring.testOnly_verifyInvariantsForNonSlices())
        XCTAssertEqual(1, ring.removeFirst())
        XCTAssertTrue(ring.testOnly_verifyInvariantsForNonSlices())
    }

    func testAddRemoveInALoop() throws {
        var ring = CircularBuffer<Int>(initialCapacity: 8)
    ...
    func testAddAllRemoveAll() throws {
        var ring = CircularBuffer<Int>(initialCapacity: 100)
        XCTAssertTrue(ring.isEmpty)
        XCTAssertEqual(0, ring.count)

        for f in 1..<100 {
            do { try ring.append(f) }
            catch { XCTFail("Should have been able to append") }
    ...
    func testRemoveAt() throws {
        var ring = CircularBuffer<Int>(initialCapacity: 7)
        for idx in 0..<7 {
            do {  try ring.prepend(idx) }
            catch { XCTFail("Should have been able to prepend") }
            XCTAssertTrue(ring.testOnly_verifyInvariantsForNonSlices())
        }

        XCTAssertEqual(7, ring.count)
        ring.remove(at: ring.index(ring.startIndex, offsetBy: 1))
        XCTAssertTrue(ring.testOnly_verifyInvariantsForNonSlices())
    ...
    func testRemoveAtLastPosition() {
        ...
        for idx in 0..<7 {
            do { try ring.prepend(idx) }
            catch { XCTFail("Should have been able to prepent") }
        }
    ...
    func testRemoveAtTailIdx0() {
        ...
        do {
            ...
            XCTFail("Should have been able to prepend")
        }
