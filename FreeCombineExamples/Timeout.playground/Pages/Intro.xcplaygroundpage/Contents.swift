...
/*:
 ...
 1. If task is cancalled before either completes, the task returns CancellationError
 1. The returned task does _not_ wait on cancelled subtasks to complete
 ...
 Structured concurrency as of today means: TaskGroup, `async let` and actors. We don't expect to be able to use them for the function above for exactly the reasons mentioned in SE-304.

 Structured concurrency don't make sense in (at least) the following circumstances:
 ...
 1. Tasks of signature: Task<Task<Output, _>, _> (flatMap and join)
